...
def match_ends(words):
    """Эта функция принимает список из строк,
    проверяет, что длина каждой строки больше
    или равна 2 и первый с посленим символом совпадают.
    После чего возвращает кол-во строк подходящих под условие.
    :param words: list
    :return match_ends: int
    """
    strings = 0
    for x in words:
...
def front_x(words):
    """Эта функция принимает список из строк,
    создаёт 2 пустых списка, в первый добавляет
    строки, которые начинаются с 'x', а во второй
    остальные. После чего сортирует их и складывает
    в один.
    :param words: list
    :return front_x: list
    """
    ...
    for x in words:
        if x[0] == 'x':
            sorted_words_1.append(x)
        if x[0] != 'x':
            sorted_words_2.append(x)
    return sorted(sorted_words_1) + sorted(sorted_words_2)
...
def sort_last(lists):
    """Эта функция принимает список и возвращает
    список отсортированный по возрастанию последнего
    элемента каждого подсписка.
    :param lists: list
    :return sort_last: list
    """
...
def remove_adjacent(nums):
    """Эта функция принимает список, в цикле
    проверяет совпадает ли символ с иднексом не равным 0
    с предыдущим и доавбляет его в пустой список, который
    потом и выводит.
    :param nums: list
    :return s: list
    """

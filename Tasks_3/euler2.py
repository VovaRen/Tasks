...
def diff():
    # return ((sum(range(101)))**2) - sum(map(lambda x: x**2, range(1, 101)))
    return ((sum(range(101)))**2) - sum([x**2 for x in range(1, 101)])
...
def max_five(nums):
    ...
    result = 1
    compositions = []
    while index_of_slice + 6 <= len(nums):
        for element in nums[index_of_slice:index_of_slice+6]:
            result = int(element) * result
        compositions.append(result)
        index_of_slice += 1
    return max(compositions)
...
def summm():
    return sum(int(x) for x in ''.join(str(2**1000)))
...
def factorial():
    ...
    nums_sum = sum(int(i) for i in str(fact))
    return nums_sum
